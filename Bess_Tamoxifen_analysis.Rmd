...
ggplot() +
  ...
  theme_bw() +
  labs(title = "HMGC279 Sum Data", x = "MDS1", y = "MDS2") +
  annotate("text", x = Inf, y = Inf, label = bquote("Stress ="~.(round(GUS_species_nmds$stress, digits = 2))), hjust = 1, vjust = 1)
```
For the figure above, GUS gene abundances were summed together if they belonged to the same taxonomy.  

```{r echo=FALSE, warning=FALSE}
GUS279_MDS = metaMDS(HMGC279)
HMGC279_vectors = envfit(GUS279_MDS, HMGC279)
...
ggplot() +
  ...
  geom_text_repel(data = feature_coords, 
            ...
            hjust = -0.2, vjust = 1,  color = "gray30", size = 3) +
  geom_text(data = GUS279_MDS$points, aes(x = MDS1, y = MDS2, label = rownames(GUS279_MDS$points))) +
  theme_bw() +
  labs(title = "HMGC279", x = "MDS1", y = "MDS2") +
  annotate("text", x = Inf, y = Inf, label = bquote("Stress ="~.(round(GUS279_MDS$stress, digits = 2))), hjust = 1, vjust = 1)
```
For the figure above, GUS gene abundances were analyzed separately, even if they belonged to the same taxon.  
...
GUS3013_MDS = metaMDS(HMGI3013)
HMGI3013_vectors = envfit(GUS3013_MDS, HMGI3013)

# Extract feature vectors and filter for significance
feature_coords = as.data.frame(scores(HMGI3013_vectors, "vectors")) * ordiArrowMul(HMGI3013_vectors)
feature_coords$pvals = HMGI3013_vectors$vectors$pvals
feature_coords = filter(feature_coords, pvals < 0.05) %>%
...
ggplot() +
  ...
  geom_text_repel(data = feature_coords, 
            ...
            hjust = -0.2, vjust = 1,  color = "gray30", size = 3) +
  geom_text(data = GUS3013_MDS$points, aes(x = MDS1, y = MDS2, label = rownames(GUS3013_MDS$points))) +
  theme_bw() +
  labs(title = "HMGI3013", x = "MDS1", y = "MDS2") +
  annotate("text", x = Inf, y = Inf, label = bquote("Stress ="~.(round(GUS3013_MDS$stress, digits = 2))), hjust = 1, vjust = 1)
```
Only vectors with taxonomy shown. There were many vectors without taxonomy.  
...
# Correlations
No normalization was performed on the protein assay values. The three readings were averaged together. Samples 21 and 26 are missing due to not being able to assign the proper protein assay values to them.   
```{r}
protein_assay_data = read.csv("Glucorunide_incubation_GUS.csv") %>%
  filter(complete.cases(.)) %>%
...
```{r}
create_heatmap(GUS_species_df, protein_assay_data)
```

HMGC279 data now.  
```{r}
create_heatmap(HMGC279, protein_assay_data)
```

HMGI3013 data.  
```{r}
test = create_heatmap(HMGI3013, protein_assay_data)
```

TODO:
- Tell the fxn to output the dataframe so that I can plot the individual relative abundances.
- Have an option to output a heatmap showing only the significant correlations with the little star indicating such.
- Plot the individual abundances of significant features.

-Eventually will need to clean up and write a methods and report.  

- Ask for 16S data to resolve 2126 and 2621 information.

In terms of protein assay data, sample 21 is similar to 14 and 17.
sample 26 is more similar to 19, 20, and 25
